import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faSearch } from "@fortawesome/free-solid-svg-icons";
import { useState } from "react";

function SearchBar({ onSearch, movieNames, isLoading, setUpdateBody }) {
  const [showDropDown, setShowDropdown] = useState(false);

  function submitSearch(e) {
    e.preventDefault();
    setShowDropdown(false);
  }

  // isLoading = true;

  return (
    <div className="relative w-full bg-slate-200 flex justify-center items-center m-5 dark:bg-slate-900 ">
      <form
        onSubmit={submitSearch}
        className="bg-slate-50 max-w-[90vw] rounded-full relative px-14 py-2 flex h-[100%] justify-center items-center gap-2 m-5 dark:bg-slate-500 "
      >
        <input
          onBlur={() => setShowDropdown(false)}
          type="text"
          placeholder="Search for a movie..."
          onChange={(e) => {
            setShowDropdown(true);
            setTimeout(() => onSearch(e.target.value), 3000);
          }}
          onKeyDown={(e) => {
            if (e.key === "Escape") {
              setShowDropdown(false);
            } else if (e.key === "Enter") {
              setUpdateBody(true);
            }
          }}
          className="outline-0 border-none p-2 text-black max-md:text-xl"
        />
        <FontAwesomeIcon icon={faSearch} />

        {showDropDown &&
          (isLoading ? (
            <div className="absolute top-[60px] left-5 z-10 lg:w-[22vw] bg-gray-100 rounded-[10px] py-3 overflow-hidden dark:bg-slate-800">
              {Array.from({ length: 3 }, (_, i) => (
                <div
                  className={`w-full px-4 py-3 animate-pulse ${i % 2 == 1 ? "bg-slate-500" : ""}`}
                  key={i}
                >
                  Loading...
                </div>
              ))}
            </div>
          ) : (
            <div className="absolute top-[60px] left-5 z-10 lg:w-[22vw] bg-gray-100 rounded-[10px] py-3 overflow-hidden dark:bg-slate-800">
              {movieNames.length > 0 ? (
                movieNames.map((movie, index) => (
                  <div
                    key={movie.id}
                    className={`mx-0 flex gap-2 py-1 items-center ${
                      index % 2 === 0 ? "" : "bg-gray-200 dark:bg-slate-900"
                    } w-full hover:bg-slate-700 transition-colors duration-75`}
                    onClick={() => console.log(movie.title)}
                  >
                    <img
                      src={`${import.meta.env.VITE_TMDB_IMAGE_URL}${movie.poster_path}`}
                      alt={movie.title}
                      className="h-15 ml-1 rounded-[5px] cursor-pointer"
                    />
                    <p className="cursor-pointer self-start">{movie.title}</p>
                  </div>
                ))
              ) : (
                <div className="px-10">
                  <p>No movies found</p>
                </div>
              )}
            </div>
          ))}
      </form>
    </div>
  );
}

export default SearchBar;
